...
#define ASTARLIBRARY_H_INCLUDED
	...
	int openList[AStarmapWidth*mapHeight+2]; //1 dimensional array holding ID# of open list items
	int whichList[AStarmapWidth+1][mapHeight+1];  //2 dimensional array used to record
// 		whether a cell is on the open list or on the closed list.
	int openX[AStarmapWidth*mapHeight+2]; //1d array stores the x location of an item on the open list
	int openY[AStarmapWidth*mapHeight+2]; //1d array stores the y location of an item on the open list
	int parentX[AStarmapWidth+1][mapHeight+1]; //2d array to store parent of each cell (x)
	int parentY[AStarmapWidth+1][mapHeight+1]; //2d array to store parent of each cell (y)
	int Fcost[AStarmapWidth*mapHeight+2];	//1d array to store F cost of a cell on the open list
	int Gcost[AStarmapWidth+1][mapHeight+1]; 	//2d array to store G cost for each cell.
	int Hcost[AStarmapWidth*mapHeight+2];	//1d array to store H cost of a cell on the open list
	int pathLength[numberPeople+1];     //stores length of the AStarFound path for critter
...
//	If not off the map (do this first to avoid array out-of-bounds errors)
	if (a != -1 && b != -1 && a != AStarmapWidth && b != mapHeight){
...
//	If so, change the parent of the cell and its G and F costs.
	else //If whichList(a,b) = onOpenList
	{
		...
		if (tempGcost < Gcost[a][b]) //if G cost is less,
		{
			...
			{
			if (openX[openList[x]] == a && openY[openList[x]] == b) //item AStarFound
			{
