...
void PathNodePool::Clear()
{
    ...
    blocks =
        firstBlock; // Don't delete the first block (we always need at least that much memory.)
                                    ...
                                    PathNode *_parent)
{
    ...
    while (root)
    {
        ...
        {
            if (root->frame ==
                frame) // This is the correct state and correct frame.
                break;
                                   ...
                                   MP_VECTOR<NodeCost> *pNodeCost)
{
    ...
    else
    {
        ...
        NodeCost *pNodeCostPtr = &(*pNodeCost)[0];
        pathNodePool.GetCache(node->cacheIndex, node->numAdjacent,
                              pNodeCostPtr);
            ...
            for (int i = 0; i < node->numAdjacent; ++i)
            {
                ...
                if (inEither)
                {
                    if (newCost < child->costFromStart)
                    {
                        ...
                        child->estToGoal =
                            graph->LeastCostEstimate(child->state, endNode);
                        child->CalcTotalCost();
                        if (inOpen)
                        {
                            open.Update(child);
                                    ...
                                    MP_VECTOR<StateCost> *near, float maxCost)
{
	...
	*/
    ...
    PathNode *newPathNode =
        pathNodePool.GetPathNode(frame, startState, 0, 0, 0);
    open.Push(newPathNode);
    ...
    while (!open.Empty())
    {
        ...
        for (int i = 0; i < node->numAdjacent; ++i)
        {
            ...
            PathNode *inOpen =
                nodeCostVec[i].node->inOpen ? nodeCostVec[i].node : 0;
            PathNode *inClosed =
