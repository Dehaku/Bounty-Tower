...
public:
        ...
        void BodyPartFind(std::string Part, int amount)
        {
            ...
            if (LinePosOne != std::string::npos)
            {
                ...
                if (LinePosTwo != std::string::npos)
                {
                    ...
                    out << amount;
                    amo = out.str();
                    BodyParts.replace(LinePosOne + len,
                                      LinePosTwo - (LinePosOne + len),
                                      out.str());
                        ...
                        std::set<int> exception = std::set<int>())
    {
        ...
        {
            Degrees =
                Math.AngleBetweenVectors(sf::Vector2f(xpos, ypos), TargetPos) -
                90;
            FirstStrike = true;
        }
        for (int i = 0; i != 30; i++)
    ...
    void DirMove(sf::Vector2f Tar)
    {
        ...
        else
        {
            ...
            int numz0 = CloseishS(Tar.y, ypos);
            int numz1 = CloseishS(Tar.x, xpos);
    ...
    { // Returns the final altered value of the altered 'aliment'
        ...
        if (tStart != std::string::npos)
        {
            ...
            if (tEnd != std::string::npos)
            {
                ...
                StrNum.append(bloodcontent, tStart + len,
                              tEnd - (tStart + len));
...
public:
    ...
    void bloodwork(std::string aliment, int amount)
    {
        ...
        if (found != string::npos)
        {
            ...
            if (foundz != string::npos)
            {
                ...
                out << amount;
                amo = out.str();
                bloodcontent.replace(found + len, foundz - (found + len),
                                     out.str());
                //cout<<"Out is:"<<out.str()<<endl;
                if (out.str() == "0")
                {
    ...
    void AddItem(char *itemname, int amount)
    { // TODO: Set this to optionally receive an item class instead, Will be useful for modded weapons and ect.
        if (Debug)
        {
            std::cout << "Pre var in AddItem"
    ...
    void InitializeCritters()
    {
        ...
        if (Input.is_open())
        {
            while (Input.good())
            {
                ...
                Critter.canmove =
                    Booleanize(StringFindNumber(line, "[CanMove:"));
                if (Debug)
                {
                    std::cout << "1 \n";
                }
                ...
                Critter.breathtimerint =
                    StringFindNumber(line, "[BreathTimerInt:");
                Critter.hungertimerint =
                    StringFindNumber(line, "[HungerTimerInt:");
                Critter.thirsttimerint =
                    StringFindNumber(line, "[ThirstTimerInt:");
                ...
                Critter.Skills.strength =
                    randz(StringFindNumber(line, "[MinStr:"),
                ...
                Critter.Skills.wisdom =
                    randz(StringFindNumber(line, "[MinWis:"),
                          StringFindNumber(line, "[MaxWis:"));
                Critter.Skills.charisma =
                    randz(StringFindNumber(line, "[MinCha:"),
                          StringFindNumber(line, "[MaxCha:"));
                Critter.Skills.perception =
                    randz(StringFindNumber(line, "[MinPer:"),
                          StringFindNumber(line, "[MaxPer:"));
                Critter.Skills.agility =
                    randz(StringFindNumber(line, "[MinAgi:"),
